...
점수화 기준:
- prior_art_overlap:
  - 30점: 겹치는 선행문헌이 없거나 1건 수준이고, 겹치는 부분도 주변 구성에 가까우며 핵심 차별점이 명확함
  - 22점: 1~2건의 선행문헌에서 일부 핵심 구성이 겹치지만, 대상 특허의 차별 구성 또는 결합 방식이 남아 있음
  - 10점: 3건 이상 선행문헌에서 유사 구성이 반복되거나, 1건의 선행문헌이 독립항 주요 구성을 상당 부분 포함함
...
- portfolio_connection:
  - 6점: 동일 제품·공정·서비스 또는 기술 로드맵 안에서 관련 특허군 3건 이상과 구체적 연결 관계 확인
  - 4점: 같은 기술분야 또는 인접 기능의 관련 특허 1~2건 확인
  - 2점: 넓은 기술분야 또는 제품군 수준의 유사성만 확인
  - 0점: 관련 특허군과의 실질적 연결성 확인 어려움
  - unknown: portfolio_context 또는 관련 특허군 정보 부족
- portfolio_coverage_extension:
  - 10점: 관련 특허군이 다루지 않는 별도 기술 요소·처리 단계·시스템 구성요소를 2개 이상 커버
  - 7점: 관련 특허군 안에서 새로운 보호 포인트 1개 이상 추가
  - 4점: 관련성은 있으나 추가 커버리지가 작거나 역할이 좁음
  - 0점: 독립적인 추가 보호 포인트가 확인되지 않거나 기존 관련 특허군과 실질 중복
