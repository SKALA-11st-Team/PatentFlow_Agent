...
- 단순히 명칭만 다르거나 적용 분야만 다른 경우에는 구성 차이로 과대평가하지 않는다.
...
- 처리 순서, 판단 로직, 제어 방식, 데이터 흐름의 차이가 확인될 때 동작 방식 차이로 본다.
...
- 효과 문구가 일반적이면 고득점하지 말고, 대상 특허의 구성·동작 차이에서 직접 도출되는 효과인지 확인한다.
...
점수화 기준:
구성 요소 구체성 (15점)
- 15: 구성 요소와 각 역할·상호관계가 구체적으로 설명됨
- 10: 주요 구성 요소와 역할은 설명되나 상호관계 또는 입력·출력이 일부 제한적임
- 5: 구성 요소 명칭은 있으나 역할 설명이 추상적임
- 0: 구현에 필요한 구성 요소를 확인하기 어려움

처리 절차 구체성 (15점)
- 15: 단계별 처리 절차와 조건·흐름이 구체적으로 설명됨
- 10: 주요 단계는 설명되나 조건, 분기, 데이터 흐름이 일부 제한적임
- 5: 처리 순서가 개략적으로만 제시됨
- 0: 단계별 처리 절차를 확인하기 어려움

구현 설명 구체성 (10점)
- 10: 기술 동작 방식, 산식, 파라미터, 실시예, 도면 등 구현자가 참고할 설명이 충분함
- 5: 구현 방향은 알 수 있으나 세부 산식, 조건, 예시, 변형 설명이 제한적임
- 0: 구현 방식 설명이 추상적이거나 확인되지 않음
...
- `component_specificity`: 0, 5, 10, 15 중 하나
- `procedure_specificity`: 0, 5, 10, 15 중 하나
- `implementation_specificity_detail`: 0, 5, 10 중 하나
