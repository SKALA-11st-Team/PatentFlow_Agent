...
- 특허 문헌은 논문 또는 실험보고서가 아니므로 정량 성능 검증, benchmark, 학습 데이터 세부 부재를 기술적 약점이나 감점 사유로 작성하지 않는다.
...
- 정량 성능 검증, benchmark, 학습 데이터 세부가 입력에 없다는 이유만으로 `risk_factors` 또는 `missing_information`에 작성하지 않는다.
