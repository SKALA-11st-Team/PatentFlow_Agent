...
- `subscores`가 권리안정성(right_stability), 권리보호력(claim_protection), 포트폴리오·방어가치(portfolio_defensive_value) 흐름과 맞습니다.
- 총점은 권리안정성 40점, 권리보호력 40점, 포트폴리오·방어가치 20점 구조와 모순되지 않습니다(합계 100). 세부 배점은 평가 프롬프트 기준을 따르며, max_score 값만으로 구조 불일치라고 단정하지 않습니다.
- 권리성 판단은 제품 적용 여부, 시장 수요, SK AX 사업 적합성과 섞이지 않습니다.

## 중점 검토 기준
1. 권리안정성 (선행문헌 기반 충돌 리스크 + 독립항 구성 명확성)
   - 선행문헌 후보 존재만으로 무효 리스크를 단정하지 않았는지 확인합니다.
   - 청구항 핵심 구성과 선행문헌의 겹침 정도를 근거로 설명했는지 확인합니다.
   - 독립항 구성 명확성은 청구항 개수가 아니라 해석 가능성(지시관계·용어 일관성·명세서 뒷받침)으로 판단했는지 확인합니다.
   - 삭제 청구항, 보정 전 청구항, 심사기록 부재를 감점 근거로 쓰지 않았는지 확인합니다.

2. 권리보호력 (핵심구성 반영도 + 유효 보호범위 + 종속항·유형 보완성 + 침해 외부확인)
   - 독립항의 필수 구성과 발명의 핵심 해결수단을 연결해 판단했는지 확인합니다.
   - 청구항 수만으로 보호력을 높거나 낮게 판단하지 않았는지 확인합니다.
   - 특정 구현 조건이 보호범위에 미치는 영향을 과장하지 않았는지 확인합니다.
   - 침해 외부확인 용이성은 구성요소의 외부 관찰 가능성(external/inferable/internal)에 근거했는지 확인합니다.

3. 포트폴리오·방어가치
   - 관련 특허군 확장성, 후속 권리화 신호, 해외 권리 확보를 구분했는지 확인합니다.
