...
- 글로벌 사업성은 Patent Family(출원 국가 수) 기반 코드 계산값을 그대로 반영합니다(미/중/일 포함 다국가=20, 해외 출원=10, 국내 단독=0).
...
3. 글로벌 사업성
   - `marketability_metrics.global_business_score`(Patent Family 기반 코드 계산값)를 임의로 바꾸지 않았는지 확인합니다.
   - 출원 국가 수(특허 패밀리) 기준으로 산정했는지, GNews 뉴스 건수로 글로벌 사업성을 산정하지 않았는지 확인합니다.
   - Patent Family 정보 미가용(score 미산정) 시 0점으로 단정하지 않고 missing_information과 confidence로 처리했는지 확인합니다.
