...
1. 기술 차별성
   ...
   - 가장 가까운 비교문헌을 식별하고 대상·비교문헌의 핵심 구성, 처리 단계, 효과를 같은 단위로 대조했는지 확인합니다.
   - “의미 있는 차이” 같은 결론만 쓰지 않고 실제로 다른 구성·관계·단계·조건을 설명했는지 확인합니다.
   ...
   - 각 세부항목에 `assessment_status`, 대상 근거, 비교 근거, 구체적 `score_reason`이 있는지 확인합니다.
   - `evaluated` 상태인데 `target_basis`가 비어 있거나, 기술 차별성의 `comparison_basis`가 비어 있으면 `valuation_retry`입니다.
   - 가장 가까운 비교문헌 식별값이 실제 `technology_metrics.similar_patents` 입력에 존재하는지 확인합니다.
   - 효과 차별성에 구성·동작 차이와 효과 사이의 인과관계가 설명되어 있는지 확인합니다.
   - 모방·회피 난이도에 대체 가능 요소, 대체하기 어려운 요소 또는 동일 효과 재현에 필요한 기술 작업이 설명되어 있는지 확인합니다.
...
2. 구현 구체성
   ...
   - `document_indicators`의 키워드 언급 횟수만으로 점수를 정하지 않고 실제 청구항·상세설명·도면 내용을 확인했는지 검토합니다.
   - 각 세부항목의 `target_basis`, 확인된 설명, 누락된 설명, `score_reason`이 점수 구간과 일치하는지 확인합니다.

3. 자료 부족 처리
   - 비교 문헌 PDF가 없거나 제목/초록만 있는 경우 그 한계를 `missing_information` 또는 confidence에 반영했는지 확인합니다.
   - 자료 부족 때문에 0점인 경우 이를 실제 기술 열위나 모방 용이성처럼 단정하지 않았는지 확인합니다.
...
- 세부점수는 있으나 대상·비교 근거 또는 점수 선택 이유가 없습니다.
- `score_reason`이 "차별성이 있음", "구체적임", "난이도가 높음" 같은 추상 표현뿐입니다.
- `insufficient_evidence`인데 `missing_information`이 비어 있습니다.
